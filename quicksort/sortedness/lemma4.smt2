...
; lemma 6
(assert
  (par (a) (forall ((x a) (y a) (xs (list a)) (ys (list a)))
    (=> 
      (and 
        (not (less a x y)) 
        (list_ge_elem a xs y) 
      ...
      (list_ge_elem a (append a xs (Cons a x ys)) y)
    )
)))

; These lemmas are not needed but we get a proof much faster with them 
; lemma 7
; (assert
;   (par (a) (forall ((x a) (y a) (xs (list a)) (ys (list a)))
;     (=> (list_ge_elem a (Cons a x xs) y) (list_ge_elem a (filter_less a x xs) y))
; )))

; ; lemma 8
; (assert
;   (par (a) (forall ((x a) (y a) (xs (list a)) (ys (list a)))
;     (=> (list_ge_elem a (Cons a x xs) y) (list_ge_elem a (filter_ge a x xs) y))
; )))

; lrs+10_1_drc=encompass:ind=struct:sik=recursion:to=lpo:thsq=on:sp=occurrence:nui=on:indao=on_89
; lemma 4
(assert-not
  (par (a) (forall ((x a) (y a) (ys (list a)))
